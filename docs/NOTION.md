...
| WhatsApp | u Mety platný trvalý token, aplikace FLEK přihlášená k odběru, profil s logem, číslo FLEKu uložené; chybí ověřovací token pro webhook, schválení pěti šablon a zkouška na telefonu, do té doby je kanál serverově vypnutý a v UI skrytý |
...
- [ ] **Dokončit WhatsApp u Meta.** Hotovo 18. 9. (Claude s tvým svolením): podmínky Meta pro WhatsApp odsouhlasené, založený testovací WhatsApp účet s testovacím číslem +1 555 156 7838; Phone Number ID a ID účtu ti Claude napsal do chatu (do repozitáře nepatří). **Stav 24. 9. ve 23:30:** trvalý token systémového uživatele funguje. Claude přihlásil aplikaci FLEK k odběru zpráv, nastavil profil na WhatsAppu (texty, web, e-mail podpory, logo) a uložil číslo FLEKu do aplikace. Všech pět šablon čeká na schválení u Mety (jednu Meta zamítla, má nové znění a je znovu ve schvalování). Zbývá:
  1. Supabase → Edge Functions → Secrets: přidej `WHATSAPP_VERIFY_TOKEN` = libovolný náhodný řetězec aspoň 16 znaků (nikam jinam ho nepíšeš). Pak napiš Claudovi, nastaví webhook.
...
- [ ] WhatsApp: `WHATSAPP_VERIFY_TOKEN` do Supabase a vlastní číslo do „To“ (Jakub), pak webhook, schválení šablon, zpřístupnění a zkouška na telefonu (token, odběr, profil s logem a číslo hotové 24. 9.)
...
| Datum | Změna |
| --- | --- |
| 24. 9. 2026 | WhatsApp u Mety je skoro hotový: s trvalým tokenem je FLEK přihlášený k odběru zpráv, profil na WhatsAppu má texty a logo FLEKu a číslo je uložené v aplikaci. Pět šablon čeká na schválení, pro webhook chybí jeden klíč v Supabase. |
