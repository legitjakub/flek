...
Implementované a nasazené v databázi i Edge Functions (15. 9. 2026), **zapnuté pro demo podniky**: přepínač `private.settings.manual_confirmation_enabled` je od 15. 9. 15:27 `demo`. Chybějící události `payment_intent.amount_capturable_updated`, `payment_intent.canceled` a `payment_intent.payment_failed` doplnila do testovacího webhooku admin funkce `stripe-webhook-setup`; akceptace v režimu potvrzování 106/106 a průchod se skutečnými testovacími platbami prošly. Přepnutí na `true` automatický režim agentovi zamítl, čeká na výslovný souhlas. Hodnota `demo` zapne tok jen pro podniky, jejichž všichni členové mají `@flek.test`; `true` pro všechny. Rozběhnuté žádosti po vypnutí dokončí worker.
