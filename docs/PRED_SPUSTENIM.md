...
- [ ] **Meta — WhatsApp Business Platform:**
  ...
  - **Dokončení teď vede administrace:** `/admin/nastaveni` → **WhatsApp → Zkontrolovat stav**. Ukáže, které tajné klíče chybí (jen jména), číslo a jméno od Mety s tlačítkem **Uložit číslo do FLEKu** (nahrazuje bod 6), kam Meta posílá webhook i s adresou ke zkopírování, odběr zpráv účtu s tlačítkem **Přihlásit odběr**, profil s tlačítkem **Nastavit profil FLEKu** (texty v brandu, web a e-mail podpory) a stav šablon; **Zpřístupnit WhatsApp** se odemkne, až je vše zelené. Fotka profilu a ikona aplikace pro Metu jsou v `docs/assets/brand/` (`whatsapp-profil-640.png`, `meta-ikona-aplikace-1024.png`).
  - V konzoli Mety zbývá jen to, co jinde udělat nejde: webhook (bod 3), fotka profilu ve WhatsApp Manageru, u testovacího čísla příjemci v „To“ a v App Settings → Basic ikona, název FLEK a zásady `https://www.app-flek.eu/soukromi` (podmínky zatím nejsou účinné, proto je tam nedávat).
