...
- [ ] **Meta — WhatsApp Business Platform:**
  - **Stav 24. 9. večer:** v Supabase secrets je token, ID účtu (WABA) i App Secret (sonda na webhook vrátila „Invalid signature“, ne „Not configured“) a zakládání šablon z administrace proběhlo ve 21:31 bez chyby. Meta zatím nevolá náš webhook a číslo FLEKu v databázi chybí, proto je WhatsApp v aplikaci skrytý.
  - **Stav 24. 9. ve 22:34:** token v Supabase (`WHATSAPP_ACCESS_TOKEN`) už neplatí — Meta odpověděla „The session is invalid because the user logged out“. Byl to dočasný token navázaný na přihlášení do Facebooku. **Jakub:** Meta Business → Nastavení firmy → Uživatelé → Systémoví uživatelé → Přidat (např. „FLEK server“, role Admin) → Přiřadit prostředky: aplikaci FLEK a WhatsApp účet (plná kontrola) → Vygenerovat token: aplikace FLEK, platnost „Nikdy“, oprávnění `whatsapp_business_messaging` a `whatsapp_business_management` → token vložit do Supabase → Edge Functions → Secrets jako `WHATSAPP_ACCESS_TOKEN` (nikdy do chatu). Tamtéž zkontrolovat `WHATSAPP_PHONE_NUMBER_ID`, `WHATSAPP_VERIFY_TOKEN` (náhodný řetězec aspoň 16 znaků) a pět názvů šablon `WHATSAPP_TEMPLATE_*` (bod 5). Zbytek dokončí agent sám přes funkci níž.
  - **Stav 24. 9. ve 23:30:** trvalý token systémového uživatele funguje (patří k aplikaci FLEK). Agent z databáze přihlásil aplikaci FLEK k odběru účtu, nastavil texty profilu, web, e-mail podpory i logo a uložil číslo FLEKu `+15551567838` (audit `whatsapp_number_changed`). Všech pět šablon čeká na schválení; `flek_business_confirmed` Meta nejdřív zamítla jako `INCORRECT_CATEGORY`, znění je přepsané (bod 4) a poslané znovu. Ve 23:33 doplnil Jakub `WHATSAPP_VERIFY_TOKEN` a agent nastavil webhook (Meta ho ověřila, posílá na `whatsapp-webhook`, pole `messages`). Zbývá schválení šablon (třetí znění `flek_business_confirmed` odesláno ve 23:37), vlastní číslo v seznamu „To“ a zkouška na telefonu; kanál se zapne až potom a na Jakubův pokyn.
  - **Stav 25. 9. v 11:06 — proč šablony čekají:** `whatsapp-templates-setup` nově vrací `health_status` účtu od Mety a ten je **BLOCKED** ze tří důvodů, které jde vyřešit jen v Meta Business Suite (platba a doklady; agent je dělat nemůže ani nemá). Kontrola účtu (`account_review_status`) je APPROVED, číslo CONNECTED a LIVE. Totéž ukazuje administrace v řádku **Účet u Mety**. **Jakub, v tomhle pořadí:**
    1. **Profil firmy (kód 131000, kvůli němu šablony stojí):** Meta Business Suite → Nastavení → Informace o firmě: právní název `Jakub Hrnčíř` (nebo obchodní firma, až bude), země Česko, web `https://www.app-flek.eu`, adresa `Sinkulova 25, 147 00 Praha 4`, e-mail `jakub@app-flek.eu`. Meta píše, že dokud tyhle tři údaje (právní název, země, web) chybí, neprovede kontroly.
    2. **Platební metoda (kód 141006):** Meta Business Suite → Nastavení → Platby → WhatsApp účet „Test WhatsApp Business Account“ → Přidat platební metodu (karta). Bez ní Meta neposílá žádné zprávy, které začíná FLEK, tedy ani šablony. Zprávy ze šablon Meta účtuje podle svého ceníku.
    3. **Ověření firmy (kód 141010, stav `pending_submission`):** Nastavení → Centrum zabezpečení → Ověření firmy → Zahájit; doklad o podnikání (živnostenský list nebo výpis z rejstříku s IČO), ověření e-mailem nebo telefonem. Bez IČO ověření nejspíš neprojde, proto souvisí s bodem „Údaje provozovatele“.
    4. **Vlastní číslo:** testovací +1 555 156 7838 píše jen na nejvýš pět čísel v API Setup → To. Pro skutečné podniky a zákazníky: WhatsApp Manager → Telefonní čísla → Přidat (číslo, na kterém ještě neběží WhatsApp), ověřit SMS, zobrazované jméno FLEK. Nové Phone Number ID pak do `WHATSAPP_PHONE_NUMBER_ID` v Supabase a agent zbytek (odběr, webhook, profil, číslo ve FLEKu) nastaví sám.
    Po bodech 1–2 by se měly rozběhnout šablony; agent je hlídá naplánovanou kontrolou a kanál zpřístupní až na Jakubův pokyn.
  - **Agent to umí dokončit bez prohlížeče:** `whatsapp-templates-setup` volá i databáze s tajným klíčem workeru (jako doručování upozornění), takže agent přes `net.http_post` zjistí stav, přihlásí odběr (`subscribe`), nastaví webhook aplikace (`webhook`), texty profilu (`profile`) i logo FLEKu jako fotku profilu (`picture`), zkontroluje šablony, uloží číslo do `private.settings` s auditem a po zkoušce zapne kanál. ID aplikace pozná funkce z tokenu (`/app`), zadávat ho netřeba; podle seznamu odběrů to nejde, protože testovací číslo má přihlášenou i aplikaci Mety „WA DevX Webhook Events 1P App“. Zamítnutou šablonu pošle znovu `resubmit` (s `dry_run: false`) v dnešním znění z kódu.
  - **Dokončení teď vede administrace:** `/admin/nastaveni` → **WhatsApp → Zkontrolovat stav**. Ukáže, které tajné klíče chybí (jen jména), číslo a jméno od Mety s tlačítkem **Uložit číslo do FLEKu** (nahrazuje bod 6), odběr zpráv účtu s tlačítkem **Přihlásit odběr**, kam Meta posílá webhook s tlačítkem **Nastavit webhook FLEKu** (ruční postup s adresou ke zkopírování zůstal sbalený jako záloha), profil s tlačítkem **Nastavit profil FLEKu** (texty v brandu, web, e-mail podpory a logo z `public/icon-maskable-512.png`) a stav šablon; **Zpřístupnit WhatsApp** se odemkne, až je vše zelené. Ikona aplikace pro Metu a fotka profilu pro ruční nahrání jsou v `docs/assets/brand/` (`meta-ikona-aplikace-1024.png`, `whatsapp-profil-640.png`).
  - **Oprava 24. 9. v noci:** funkce dřív neodpovídala na předletový dotaz prohlížeče (CORS), takže panel z www.app-flek.eu funkci vůbec nezavolal; dosud ji spouštěl jen skript a databáze. Teď odpovídá stejně jako ostatní funkce volané z aplikace, jen pro adresy FLEKu.
  - V konzoli Mety zbývá jen to, co jinde udělat nejde: u testovacího čísla příjemci v „To“ a v App Settings → Basic ikona, název FLEK a zásady `https://www.app-flek.eu/soukromi` (podmínky zatím nejsou účinné, proto je tam nedávat).
  ...
  4. Pět šablon kategorie **Utility**, jazyk čeština (`cs`). **Neklikej je ručně:** po vložení tajných klíčů (bod 5) je založí administrace v **Nastavení → Šablony zpráv na WhatsAppu** (`/admin/nastaveni`, funkce `whatsapp-templates-setup`) — nejdřív Zkontrolovat, pak Založit chybějící; znění níž má uložené v `supabase/functions/_shared/whatsapp.ts` a šablonu, která už u Mety je, nepřepisuje. Ruční postup zůstává jako záloha. Parametry musí zůstat v tomto pořadí (posílá je `claim_whatsapp_deliveries`):
  ...
  6. Číslo FLEKu uložit v administraci tlačítkem **Uložit číslo do FLEKu** (`admin_set_whatsapp_display_number`, zapíše se do auditu) a po zelených řádcích **Zpřístupnit WhatsApp**. Tím se WhatsApp objeví v Provozovně, v Profilu a ve výzvách.
