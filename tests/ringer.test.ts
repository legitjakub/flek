...
describe('ringer', () => {
  ...
  it('sounds like two tones and a pause, loud enough but never clipping', async () => {
    const audio = fakeAudio();
    const ringer = await import('../src/features/merchant/ringer');
    ...
    // The first ding is audible straight away; the last second of the cycle is the pause.
    const loudness = (from: number, to: number) => samples.slice(from * rate, to * rate).reduce((sum, sample) => sum + sample * sample, 0);
    expect(loudness(0, 0.1)).toBeGreaterThan(1);
    expect(loudness(1.6, 2.6)).toBeLessThan(1e-3);
  });
});
