...
- **Podoba běžné ceny zůstává (25. 9.).** Šedá cena se slovem „běžně“ se nelíbila a z pěti dalších návrhů Jakub žádný nevybral: běžná cena zůstává červeně přeškrtnutá vedle ceny FLEKu.
...
- **Šablony věcně jako potvrzení transakce, bez „ověření kódu“.** Meta dvakrát zamítla `flek_business_confirmed` jako `INCORRECT_CATEGORY`. Obě znění měla větu o ověření kódu zákazníka, kterou Meta nejspíš čte jako přihlašovací kód (kategorie Authentication); třetí znění končí stejně jako šablona o zrušení („Podrobnosti najdete v aplikaci FLEK Partner v Rezervacích“). Parametry zůstávají stejné ve stejném pořadí, takže se nemění `claim_whatsapp_deliveries` ani názvy v Supabase. Šablona se posílá jen při vypnutém potvrzování, takže dokud je zapnuté, spuštění neblokuje. Zamítnutá šablona se upravuje pod stejným názvem (`POST /{id}`), ne zakládá znovu.
...
- **Zvoní, dokud podnik nezareaguje.** Jedno krátké pípnutí se v salonu přeslechlo a prohlížeč ho většinou vůbec nepustil, protože na stránku ještě nikdo neklepl. Jako tablety rozvozových služeb teď partnerská část zvoní smyčkou (tón FLEKu každých 2,6 s), dokud žádost nepotvrdí, neodmítne (kdekoli), nevyprší nebo ji někdo neztlumí. Ztlumení platí jen pro žádosti, které už zazněly; další zazvoní znovu. Nic se neukládá, ztlumení i rozsvícený displej trvají, dokud je stránka otevřená.
- **Tón FLEKu místo zvonku.** Zvuk je vzestupný durový akord G–C–E zahraný dvakrát měkkou paličkou a pauza (2,6 s), syntetizovaný v kódu (`ringSamples`), takže ho testy i náhled slyší stejně. „Ding-dong“ zněl jako domovní zvonek; stoupající akord zní jako dobrá zpráva a přesto je přes salon slyšet. Lišta ukazuje odpočet do vypršení první žádosti, stejnými slovy jako karta.
- **Smyčka ve Web Audio, ne časovač.** Prohlížeč brzdí časovače v kartě na pozadí až na jeden za minutu; zvuková smyčka běží ve zvukovém vlákně a zvoní dál. Zvuk se odemkne prvním klepnutím kamkoli v konzoli; dokud to nejde, lišta nabídne „Zapnout zvuk“ (a ten, kdo klepne, už žádost vidí, takže uslyší jen jedno zazvonění na zkoušku).
...
- **Nová žádost přes celou obrazovku.** Lišta nahoře se v salonu přehlédla stejně jako pípnutí; tablety Uber Eats, Wolt nebo Bolt při objednávce převezmou celou obrazovku, blikají a zvoní, dokud někdo nepotvrdí. FLEK dělá totéž ve svých barvách (`IncomingRequest.tsx`). Nativní modální `<dialog>`: je nad každým otevřeným panelem, stránku pod sebou dělá neaktivní a Esc ho zavře bez knihovny. Obrazovka rozhoduje přes stejné `respond_to_booking` jako karty, takže nevzniká nový kanál.
- **Výzva nahoře, jeden bílý panel dole.** První podoba skládala na modrou bílé bloky (kolečko, karta, tlačítko), druhá je nahradila průsvitným „lístkem“; obě působily roztříštěně. Aplikace, které tohle řeší denně, mají silnou barevnou výzvu a pod ní jeden čitelný panel s tím, co je potřeba k rozhodnutí, a jedním tlačítkem (Uber pro řidiče: panel dole s blikajícím tlačítkem, Uber Eats Orders: obrazovka zezelená, Bolt: ubývající kruh; příchozí hovor: kdo volá uprostřed). FLEK proto nahoře v modré „volá“ svým logem (ne samotným špendlíkem, aby bylo jasné, kdo volá) s kruhem odpočtu a dole má bílý panel ukotvený k okraji v barvách aplikace; na tabletu, počítači a telefonu na šířku napůl vedle sebe. Jediné barevné tlačítko je „Potvrdit rezervaci“, odmítnutí je tiché textové tlačítko.
- **Ochrana proti nechtěnému klepnutí.** Obrazovka se objeví nečekaně, třeba pod prstem, který zrovna mířil jinam. Proto se klepnutí v prvních 0,7 s nepočítá, „Nemohu přijmout“ se ještě zeptá „Opravdu odmítnout?“ a klepnutí na pozadí zvuk neodemkne (odemyká jen tlačítko zvuku a rozhodnutí; rozhodnutí žádost hned ztlumí, aby smyčka nehrála, než se seznam načte).
- **Pulzuje i bez zvuku.** Když prohlížeč zvuk ještě blokuje, je oko jediný kanál, takže logo pulzuje, dokud žádost není ztlumená. Vlny a paprsky mají rytmus zvonění (2,6 s, akord v 0 a 0,95 s). Animuje se jen `transform` a `opacity`; při omezeném pohybu vše skončí v klidu.
- **Živá žádost má přednost.** Potvrzení a odmítnutí se ukáže 4 s a samo pokračuje. Zpráva o žádosti, která skončila jinde nebo vypršela, počká na přečtení (jako zmeškaný hovor), ale jakmile čeká jiná žádost, obrazovka na ni přejde sama. „Později“ žádost vrátí do lišty zvonění (zvoní dál, dokud ji někdo neztlumí), nová žádost obrazovku převezme znovu.
...
- **Menu na tabletu jen slovy.** Na 768 px se pět položek s ikonami, logo a „Pro podniky“ nevešly a stránka se posouvala do strany. Ikony v horním menu jsou až od 1024 px; slova nesou význam a zkrácení („Rez.“) by bylo horší. Odznak nových míst v Oblíbených stojí na tabletu i počítači za slovem, na telefonu dál na ikoně.
- **Termín, který už nejde rezervovat, to řekne v kartě.** Na počítači je karta termínu vpravo vedle fotky a důvod byl pod ohybem, takže modrá karta s „Tvůj termín ✓“ tvrdila opak. Karta ztratí barvu i fajfku, řekne „Tenhle čas už nejde rezervovat“ a pod ní stojí důvod s odkazem na náhradu.
- **„Začal“ není „proběhl“.** Běžící termín dřív hlásil „už proběhl… je minulostí“, což dokud běží, není pravda. Důvod `started` teď znamená jen „už začal“, `ended` přijde až po jeho konci. Oba vycházejí jen z časů od serveru; o dostupnosti dál rozhoduje `offer_is_bookable`.
- **Malé ovladače mapy mají velkou plochu.** „Zdroje mapy“ zůstávají malé, aby nezakrývaly mapu, ale plocha pro klepnutí je 44 px (neviditelný `::after`). Tlačítka přiblížení mají 44 px vždy; pravidlo má vyšší specifičnost, protože styly MapLibre se načítají až s mapou a stejně specifické pravidlo přebily.

## Potvrzení rezervace — 25. 9. 2026

- **Světlá karta s kalendářním listem, ne lístek.** Okno před platbou byla tabulka „Co / Kde / Kdy“ s hodnotami u pravého okraje, šedý odstavec o platbě a dvě stejně hlasité poznámky, takže to hlavní (kdy, co a za kolik) nebylo o nic výraznější než drobné písmo. První nová podoba byla tmavý útržek s perforací jako karta „Tvůj FLEK“; ten se Jakubovi nelíbil a nechce motiv lístku opakovat všude (zůstává jen u karty „Tvůj FLEK“ v Profilu). Teď je nahoře jedna světlá karta: vlevo kalendářní list (den v týdnu na modrém pruhu, číslo dne a měsíc) jako u pozvánky v kalendáři, vedle služba, „Zítra 18:30–19:30“ modře s délkou v bublince a podnik s adresou, pod čarou peníze.
- **Co se stane, ve třech krocích.** Místo odstavce tři řádky s ikonou: zrušení zdarma (zeleně, uklidnění), co se teď stane s penězi a co udělá podnik. Rozlišují se jen podle `confirmation_quote`; dokud nepřijde, je místo nich zástupný blok stejné výšky, aby jeden slib neproblikl a nevyměnil se za druhý.
- **Částka na tlačítku vpravo.** Jako pokladny rozvozových a rezervačních aplikací: vlevo co se stane („Pokračovat k platbě“ se zámkem), vpravo kolik. Tlačítko je modré jako „Chytit FLEK“, na které navazuje. Pod ním jediná věta o tom, jak dlouho termín držíme; chyba se ukazuje nad tlačítkem, protože na konci rolovaného obsahu ji telefon schoval.
- **Lhůta vždy se dnem.** „Zrušení zdarma do 17:30“ ukázané v 18:16 u zítřejšího termínu vypadalo jako lhůta, která už uplynula. `untilLabel` píše „dnes do 17:30“, „do zítřka 17:30“ nebo „do 28. 9. 17:30“, a to v okně rezervace, v detailu nabídky i v Rezervacích.
- **Kalendářní list, ne fotka.** Vlevo je den, ne zmenšená fotka: ta je na stránce nabídky hned pod oknem, ilustrační fotka by musela nést štítek „ilustrační foto“ a v malém čtverečku by nebyla čitelná ani fotka, ani štítek. Den je u rezervace na poslední chvíli to podstatné; pro čtečku obrazovky je celé datum v textu vedle („sobota 26. 9.“).
