...
- **Webhook a logo tlačítkem, ne v konzoli Mety.** Webhook aplikace se nastavuje přes `/{app}/subscriptions` tokenem aplikace a logo přes nahrávací API Mety (sezení u aplikace, soubor, handle do profilu). ID aplikace funkce vezme z odběru účtu, který přihlašuje tentýž token, takže ho nikdo nezadává. Logo je `public/icon-maskable-512.png` z webu FLEKu: čtverec bez průhlednosti s celým špendlíkem uvnitř kruhového ořezu, stejný jako ikona na ploše telefonu.
