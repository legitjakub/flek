...
begin
 for i in 1..12 loop
  uid:=md5('volno-user-'||i)::uuid;
  mail:=case i when 1 then 'demo-customer@volno.test' when 2 then 'demo-merchant@volno.test' when 3 then 'demo-merchant2@volno.test' when 4 then 'demo-admin@volno.test' else 'demo-'||i||'@volno.test' end;
 ...
 insert into public.user_roles values(md5('volno-user-4')::uuid,'admin');
end $$;
...
begin
 ...
 ["Studio Nová kapitola","Vinohrady","Francouzská 24",50.0731,14.4385,"vlasy","Pánský střih",650,45]
  ...
  i:=i+1;bid:=md5('volno-business-'||i)::uuid;cat:=item->>5;price:=(item->>7)::int*100;dur:=(item->>8)::int;service_name:=item->>6;
  ...
  values(bid,item->>0,'demo-'||i,'Fiktivní provozovna pro ukázku VOLNO. Klidné místo v čtvrti '||(item->>1)||', kde je čas jen pro vás.',cat,'+420777200'||lpad(i::text,3,'0'),'studio'||i||'@volno.test',item->>2,'Praha',item->>1,'12000',extensions.st_setsrid(extensions.st_makepoint((item->>4)::float8,(item->>3)::float8),4326)::extensions.geography,case when i=15 then 'pending'::public.business_status else 'approved'::public.business_status end);
  insert into public.business_members values(bid,md5('volno-user-'||case when i=1 then 2 when i=15 then 3 else 5+(i%8) end)::uuid,'owner');
  insert into public.services(id,business_id,name,description,category_slug,duration_minutes,normal_price_cents)
  values(md5('volno-service-'||i)::uuid,bid,service_name,'Dopřej si chvíli pro sebe. V ceně je vše potřebné pro tuto službu.',cat,dur,price),
  (md5('volno-service-'||(i+15))::uuid,bid,case cat when 'vlasy' then 'Úprava vousů' when 'masaze' then 'Masáž zad a šíje' when 'krasa' then 'Péče o ruce' when 'sport' then 'Individuální lekce' when 'joga' then 'Ranní protažení' else 'Odpočinková procedura' end,'Péče a pozornost bez spěchu.',cat,30,greatest(25000,(price/200)*100));
 end loop;
end $$;
...
begin
 for i in 1..38 loop
  select * into s from public.services where id=md5('volno-service-'||case when i in (30,33,34,35,36,37,38) then 1 else 1+((i-1)%14) end)::uuid;
  ...
  if i>=30 then
   for k in 1..(case when i=31 then 3 else 1 end) loop
    uid:=md5('volno-user-'||case when i=31 then k+7 when i=33 then 6 else 1 end)::uuid;
