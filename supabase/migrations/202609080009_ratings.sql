-- Ratings. Only a customer who actually showed up can rate, so the number is earned
-- rather than collected: the booking must be theirs and already resolved as completed.
alter table public.bookings
  add column rating smallint check (rating between 1 and 5),
  add column rated_at timestamptz;

create index bookings_business_rating on public.bookings(business_id) where rating is not null;

create function public.rate_booking(p_booking_id uuid, p_rating integer) returns void
language plpgsql security definer set search_path=public as $$
declare k public.bookings;
begin
 if auth.uid() is null then raise exception 'AUTH_REQUIRED'; end if;
 if p_rating is null or p_rating not between 1 and 5 then raise exception 'VALIDATION_ERROR'; end if;
 select * into k from public.bookings where id=p_booking_id and customer_id=auth.uid() for update;
 if not found then raise exception 'FORBIDDEN'; end if;
 if k.status<>'completed' then raise exception 'NOT_RATEABLE'; end if;
 update public.bookings set rating=p_rating::smallint,rated_at=now() where id=k.id;
end $$;

-- Aggregated at query time from the bookings themselves; a denormalised counter would be
-- one more thing that can drift away from the truth.
...
revoke execute on function public.rate_booking(uuid,integer) from public,anon;
grant execute on function public.rate_booking(uuid,integer) to authenticated,service_role;
