-- Mapa kreslí jeden bod na podnik, ne seznam karet, a strop 100 řádků jí bral celé ulice.
-- Při běžném filtru (do 5 km, řazení „nejblíž") vracelo 50 řádků body jen tří podniků ze
-- šestnácti: špendlíky nechyběly v kreslení, chyběly řádky v odpovědi. Při 300 řádcích dosáhne
-- na všechny podniky každé řazení. Mění se jediná mez ve validaci, tělo zůstává slovo od slova.
create or replace function public.search_offers(p_lat float8,p_lng float8,p_radius_m integer default 5000,p_category text default null,p_from timestamptz default now(),p_until timestamptz default null,p_min_discount_pct integer default 0,p_max_price_cents integer default null,p_sort text default 'recommended',p_limit integer default 20,p_offset integer default 0,p_daypart text default null)
