...
- 25. 9. 8:11 pražského času (ranní naplánovaná kontrola, `whatsapp-templates-setup` z databáze s klíčem workeru, `dry_run`, HTTP 200): všech pět šablon pořád PENDING, žádná zamítnutá (`rejected_reason` prázdné), takže nic k přepsání ani k novému odeslání; webhook čísla i odběr aplikace dál míří na `…/functions/v1/whatsapp-webhook` (pole `messages`, aktivní), aplikace FLEK přihlášená k odběru účtu; číslo `name_status` AVAILABLE_WITHOUT_REVIEW, kvalita zatím UNKNOWN. `whatsapp_enabled` zůstává vypnuté. Další kontrola naplánovaná odpoledne.
