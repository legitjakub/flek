...
import { BadgePercent, ChevronLeft, MapPin, TicketCheck } from 'lucide-react';
          ...
          >
            ...
            <footer className="shrink-0 border-t border-line bg-card px-5 pt-4 pb-[max(1rem,env(safe-area-inset-bottom))]">
              ...
              <div className="flex gap-2">
                {carousel.index > 0 ? (
                  <Button variant="secondary" aria-label="Předchozí krok" onClick={() => carousel.goTo(carousel.index - 1)}>
                    <ChevronLeft size={18} aria-hidden="true" /> Zpět
                  </Button>
                ) : null}
                <Button
                  data-intro-next
...
function IntroVisual({ index }: { index: number }) {
  if (index === 0) {
    return (
      <div className="relative mx-auto aspect-[4/3] w-full max-w-sm overflow-hidden rounded-3xl bg-accent-soft shadow-card" aria-hidden="true">
  ...
  if (index === 1) {
    return (
      <div className="mx-auto flex aspect-[4/3] w-full max-w-sm flex-col justify-between rounded-3xl bg-card p-6 shadow-card" aria-hidden="true">
        <span className="grid size-14 place-items-center rounded-2xl bg-brand text-ink"><BadgePercent size={29} /></span>
        <div>
          <p className="tnum text-3xl font-extrabold text-ink">225 Kč</p>
          <p className="mt-2 flex items-center gap-2 text-base"><s className="tnum text-muted decoration-danger decoration-2">450 Kč</s><span className="rounded-lg bg-brand px-2 py-1 text-sm font-extrabold text-ink">−50 %</span></p>
          <p className="mt-3 font-bold text-accent">Ušetříš 225 Kč</p>
        </div>
      </div>
    );
  }
  return (
    <div className="mx-auto flex aspect-[4/3] w-full max-w-sm flex-col items-center justify-center rounded-3xl bg-ink p-6 text-center text-card shadow-lift" aria-hidden="true">
