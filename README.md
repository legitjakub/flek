...
Ověření: `npm run test:unit`, `npm run build`, `npm run test:acceptance`. Poslední příkaz používá pouze veřejný klíč a demo účty a mění demo data; nespouštět proti skutečným účtům. `tests/pilot-maintenance.sql`, `tests/stripe-refunds.sql`, `tests/manual-confirmation.sql`, `tests/whatsapp-notifications.sql`, `tests/legal.sql` a `tests/oauth-profile.sql` vyžadují privilegovaný přístup, běží v transakci a své změny vracejí zpět. Akceptační skript sám pozná, jestli demo podnik potvrzuje rezervace, a projde odpovídající tok. `npm run check:oauth` nic nemění: přečte veřejné nastavení Auth a vypíše, jestli je přihlášení přes Google a Apple zapnuté a jestli je povolená návratová adresa. Historie nasazených migrací a omezení ověření jsou v [přehledu projektu](docs/PROJECT_STATUS.md).
