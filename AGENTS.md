...
| `src/features/merchant` | FLEK Partner: provozovna, služby, zveřejnění FLEKu, rezervace, metriky, upozornění; průvodce „Začínáme“ a `usePublishReadiness` (`SetupGuide.tsx`), nápověda „Jak FLEK funguje“ a sdílené kroky (`PartnerHelp.tsx`), zvonění čekající žádosti (`ringer.ts`, `RequestRing.tsx`) |
...
| `src/features/notifications` | zvonek s upozorněními a nastavení e-mailu, push a WhatsApp (zákazník v Profilu, podnik v Provozovně), zapíná `VITE_NOTIFICATIONS_ENABLED`; registrace zařízení pro push a české hlášky (`devicePush.ts`); `WhatsApp.tsx` ověření čísla jedním klepnutím a výzvy |
...
| `tests` | unit testy (Vitest), `integration.test.ts` (potřebuje Docker), `pilot-maintenance.sql`, `stripe-refunds.sql`, `manual-confirmation.sql`, `whatsapp-notifications.sql`, `legal.sql`, `oauth-profile.sql` a `watches.sql` (SQL v transakci s rollbackem) |
